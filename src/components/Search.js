...
class Search extends React.Component {
  ...
  getPlant = async e => {
    ...
    const default_img =
      "https://res.cloudinary.com/dsbgj0oop/image/upload/v1572516426/default_img.png";
    // Premier appel API
    const api_1st_call = await fetch(
      `https://trefle.io/api/plants?q=${common_name}&complete_data=true&token=${API_KEY}`
    );
    ...
    if (data[0]) {
      ...
      if (common_name && data[0]) {
        ...
        for (let i = 0; i < data.length; i++) {
          const api_2nd_call = await fetch(
            `https://trefle.io/api/plants/${data[i].id}?token=${API_KEY}`
          );
          const data_plant = await api_2nd_call.json();
          const species = data_plant.main_species
            ? data_plant.main_species.common_name
            : "undefined";
          const randImage =
            data_plant.images[0] !== undefined
              ? data_plant.images[this.getRandomInt(data_plant.images.length)]
                  .url
              : default_img;
          finalResult.push({
            id: data_plant.id,
  ...
  render() {
    ...
    return (
      <div className="search">
        ...
        <p
          className={
            visible_caption ? "plantCard-error" : "plantCard-error invisible"
          }
        >
          {`${finalResult.length} plant`}
          <span className={displayS}>s </span>
          {` found. `}
          <span className={scrollClass}>
            Please scroll to watch all the results !
          </span>
        </p>
